...
'''
For test_data we don't shuffle, for the purpose of comparison when Testing.
'''
...
for i, samples in enumerate(zip(train_monet.take(4), train_photo.take(4))):
    ...
    ax[i, 0].imshow(monet)
    ax[i, 1].imshow(photo)
plt.savefig('./Figure/dataset_example.png')
# plt.show()
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i, img in enumerate(test_monet.take(self.num_img)):
            ...
            prediction.save(
                "./Figure/generated_img_{i}_{epoch}.png".format(i=i, epoch=epoch + 1)
            )
        # plt.save("./300/generated_img_{epoch}.png".format(i=i, epoch=epoch + 1))
        plt.show()
        plt.close()
...
'''cycle_gan_model.fit is the main process of training.'''
# cycle_gan_model.fit(
...
weight_file = "./model_checkpoints/cyclegan_checkpoints.300"
cycle_gan_model.load_weights(weight_file).expect_partial()
print("Weights loaded successfully")

_, ax = plt.subplots(10, 2, figsize=(10, 25))
for i, img in enumerate(test_photo.take(10)):
    ...
    prediction = keras.preprocessing.image.array_to_img(prediction)
    prediction.save("./p300/predicted_img_{i}.png".format(i=i))
plt.tight_layout()
plt.savefig("Test300.jpg")
plt.show()